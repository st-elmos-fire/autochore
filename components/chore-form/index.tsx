...
/* Render component */
export const ChoreForm: React.FC<Props> = ({
  choresList,
  chore,
  updateChoresList,
  users,
  ...
  const [exceptionType, setExceptionType] = useState(
    chore?.exceptionType || 'none'
  );
  const [existingChore, setExistingChore] = useState(
    chore?.existingChore || ''
  );
  const [frequency, setFrequency] = useState(chore?.frequency || 'daily');
  const [day, setDay] = useState<number[] | unknown[]>([]);
  const [month, setMonth] = useState<number[] | unknown[]>([]);
  const [runOn, setRunOn] = useState<string[] | unknown[]>(chore?.run_on || []);
  ...
  const updateChore = async () => {
    if (updateType === 'add') {
      const existingChores = choresList.filter(
        (chore) => chore.content === name
      );
    ...
    if (rollingRunOn.month && rollingRunOn.day) {
      stitchedDates = rollingRunOn.month
        .map((m) => {
  ...
  return (
    <>
      <Box
        ...
        autoComplete="off"
      >
        <InputLabel htmlFor="task-name" required>
          Name
        </InputLabel>
        <TextField
          id="task-name"
        ...
        <TextField
          ...
          value={description}
        />
        <InputLabel htmlFor="effort" required>
          Effort
        </InputLabel>
        <Select
          id="effort"
        ...
        >
          ...
          {users.map((user) => (
            <MenuItem key={user.todoist_id} value={user.todoist_id}>
              {user.name}
            </MenuItem>
          ))}
        </Select>
        ...
        {choresList.length > 0 && (
          <>
            ...
            {exceptionType !== 'none' && (
              <>
                <InputLabel htmlFor="exception">
                  If the following chore exists
                </InputLabel>
                <Select
                  id="exception"
                  value={existingChore}
                ...
                >
                  ...
                  {choresList.map((chore) => {
                    return (
                      <MenuItem key={chore.content} value={chore.content}>
                        {chore.content}
                      </MenuItem>
                    );
                  })}
        ...
        <InputLabel htmlFor="frequency" required>
          Frequency
        </InputLabel>
        <Select
          id="frequency"
                      ...
                      <Checkbox
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setRollingRunOn({
                              ...rollingRunOn,
                              month: rollingRunOn.month.filter(
                                (d) => d !== m.number
                              )
                            });
                          }
